...
public class App extends Application {
  ...
  private String getSocialAPIURL() {
    return (isDeveloped() || isIDEStarted())?"http://localhost:3007":socialAPIURL;
  }
...
System.out.println("Data path: "+dataDir.getAbsolutePath());
        ...
        int currentMinorVersion=13; //версия исправлений в пределах мажорной версии currentUpdateFile
...
//        }
  ...
  private void updateIn14_9(ProgramOptions updateOption) {
    File es_translate=null;

    try {
      ResourceUtil ru=new ResourceUtil();
      es_translate = ru.saveResource(getTmpDir(),"es_translanion.xml","/updates/update_14_9/spanish_base.xml",true);
      if(es_translate==null) throw new Exception();

      LoadLanguageFiles ll=new LoadLanguageFiles();
      if( ll.parse(Arrays.asList(es_translate),getModel())){

        log.info("ОБНОВЛЕНИЕ 14.9 ЗАВЕРШЕНО.");
      }
      else  wrapException("14.9",new Exception());

      getModel().createOption("correct_14_9_update", "yes");
      //до того была ошибка, перетерся англ язык испанским. Файд xml исправлен(указан верный язык в начале).
      // Те кто получат верное обновления не должны делать следующие
    } catch (IOException e) {
  ...
  private void updateIn14_10(ProgramOptions updateOption) {
    try{
      getModel().getOption("correct_14_9_update");
    }catch (Exception ex){
      //исключение будет если предыдущее обновление было некорректным( это было исправлено в этом обновлении )+ те кто
      File es_translate=null;
      File en_translate=null;
      try {
        ResourceUtil ru=new ResourceUtil();
        es_translate = ru.saveResource(getTmpDir(),"es_translanion.xml","/updates/update_14_9/spanish_base.xml",true);
        if(es_translate==null) throw new Exception();

        en_translate = ru.saveResource(getTmpDir(),"en_translanion.xml","/updates/update_14_9/english.xml",true);
        if(en_translate==null) throw new Exception();

        LoadLanguageFiles ll=new LoadLanguageFiles();
        if( ll.parse(Arrays.asList(es_translate, en_translate),getModel())){

          log.info("ОБНОВЛЕНИЕ 14.10 ЗАВЕРШЕНО.");
        }
        else  wrapException("14.10",new Exception());

      } catch (IOException e) {
        wrapException("14.10",e);
        log.info("ОБНОВЛЕНИЕ 14.10 НЕ ЗАВЕРШЕНО.");
