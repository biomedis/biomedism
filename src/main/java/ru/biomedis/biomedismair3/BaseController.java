...
public abstract class BaseController implements Initializable
{
  ...
  public void setWindow(Stage win){window=win;}
  /**
   * Установка базового класса приложения для доступа из контроллеров.
   * @param lp 
   */
   public static void setApp(App lp) { BaseController.app=lp;}
   
   /**
    * Установка базового контролера для доступа из других контроллеров
    * @param mc 
    */
   public static void setMainController(AppController mc){mdc=mc;}
    ...
    * @return 
    */
   public  static App getApp(){return BaseController.app;} 
 ...
 dialog.initOwner(owner);
   ...
   /*
   public static void showExceptionDialog(String title,String msg, Throwable ex, Window owner,boolean lightweight)
   {
       String mh=null;
       Window onw=BaseController.getApp().getMainWindow();
       if(owner!=null)onw=owner;
      Dialogs create = Dialogs.create();
    
      //выпадает с ошибкой внутри библиотеки на showException(ex);NullPointer!!!
     if(lightweight) Dialogs.create().lightweight().owner(onw).title(title).message(msg).masthead(mh).showException(ex);
      else  Dialogs.create().owner(onw).title(title).message(msg).masthead(mh).showException(ex);
      
   }
   
    public static void showInfoDialog(String title,String msg,  Window owner,String masterHead,boolean lightweight)
   {
       String mh=null;
       Window onw=BaseController.getApp().getMainWindow();
       if(owner!=null)onw=owner;
    
      if(lightweight)   Dialogs.create().lightweight().owner(onw).title(title).message(msg).masthead(mh).showInformation();
      else Dialogs.create().owner(onw).title(title).message(msg).masthead(mh).showInformation();
      
   }
    public static void showErrorDialog(String title,String msg,  Window owner,String masterHead,boolean lightweight)
   {
       String mh=null;
       Window onw=BaseController.getApp().getMainWindow();
       if(owner!=null)onw=owner;
    
      if(lightweight)   Dialogs.create().lightweight().owner(onw).title(title).message(msg).masthead(mh).showError();
      else Dialogs.create().owner(onw).title(title).message(msg).masthead(mh).showError();
      
   }
     public static void showWarningDialog(String title,String msg,  Window owner,String masterHead,boolean lightweight)
   {
       String mh=null;
       Window onw=BaseController.getApp().getMainWindow();
       if(owner!=null)onw=owner;
    
      if(lightweight)   Dialogs.create().lightweight().owner(onw).title(title).message(msg).masthead(mh).showWarning();
      else Dialogs.create().owner(onw).title(title).message(msg).masthead(mh).showWarning();
      
   }
     public static Action showConfirmDialog(String title,String msg,  Window owner,String masterHead,boolean lightweight)
   {
       Action action=null;
       String mh=null;
       Window onw=BaseController.getApp().getMainWindow();
      ...
      if(lightweight) action=  Dialogs.create().lightweight().owner(onw).title(title).message(msg).masthead(mh).actions(new Action[] {YES, NO } ).showConfirm();
      else action = Dialogs.create().owner(onw).title(title).message(msg).masthead(mh).actions(new Action[] {YES, NO } ).showConfirm();
      return action;
   }
     
     
      public static String showStringInputDialog(String title,String msg,  Window owner,String masterHead,boolean lightweight,String initText)
        {
            String res=null;
            String mh=null;
            Window onw=BaseController.getApp().getMainWindow();
           ...
           if(lightweight) res=  Dialogs.create().lightweight().owner(onw).title(title).message(msg).masthead(mh).showTextInput(initText);
           else res = Dialogs.create().owner(onw).title(title).message(msg).masthead(mh).showTextInput(initText);
           return res;
        }
      
       public static <T> T showChoiceInputDialog(String title,String msg,  Window owner,String masterHead,boolean lightweight, T defaultChoice,Collection<T> choices)
        {
            T res=null;
            String mh=null;
            Window onw=BaseController.getApp().getMainWindow();
           ...
           if(lightweight) res=  Dialogs.create().lightweight().owner(onw).title(title).message(msg).masthead(mh).showChoices(defaultChoice,choices);
           else res = Dialogs.create().owner(onw).title(title).message(msg).masthead(mh).showChoices(defaultChoice,choices);
           return res;
        }
